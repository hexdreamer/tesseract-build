...
if ! source $SCRIPTSDIR/utility.sh; then
  echo "build_libpng.sh: error sourcing $SCRIPTSDIR/utility.sh"
  exit 1
fi
...
if {
  pkg-config --exists --print-errors 'libpng >= 1.6.37' &&
    [ -f $ROOT/ios_arm64/lib/libpng.a ]
}; then

  print 'skipped config/make/install, found ROOT/ios_arm64/lib/libpng.a'
...
if {
  pkg-config --exists --print-errors 'libpng >= 1.6.37' &&
    [ -f $ROOT/ios_x86_64/lib/libpng.a ]
}; then

  print 'skipped config/make/install, found ROOT/ios_x86_64/lib/libpng.a'
...
if {
  pkg-config --exists --print-errors 'libpng >= 1.6.37' &&
    [ -f $ROOT/macos_x86_64/lib/libpng.a ]
}; then

  print 'skipped config/make/install, found ROOT/macos_x86_64/lib/libpng.a'
